...
export function NoteCard({ note, onNoteDelete }: NoteCardProps) {
  return (
    ...
    <Dialog.Root>
      <Dialog.Trigger className='flex flex-col rounded-md text-left outline-none bg-slate-800 p-5 gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className='fixed inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md overflow-hidden flex flex-col outline-none'>
          ...
          <button type="button" onClick={() => onNoteDelete(note.id)} className='w-full bg-slate-800 py-4 text-center text-sm text-slate-300 outline-none font-medium group'>
